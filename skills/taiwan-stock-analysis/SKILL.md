...
預設啟用反錨定模式。除非使用者明確要求跟隨外部共識，估值、排名、分數、進出場條件與風險報酬判斷不得使用券商目標價、分析師評等、法人喊價、新聞標題或市場共識價作為輸入。這些資訊只能放在報告最後的「外部共識對照」段落，用來比較差異與檢查偏誤，且不得回頭修改前面的獨立結論。

需要資料來源分級或外部共識隔離規則時，讀取 `references/data-sources.md`。需要評分權重、反錨定流程或短線/低估綜合篩選邏輯時，讀取 `references/methodology.md`。
...
篩選排名必須先輸出獨立結果。若同時提供外部共識或機構目標價，只能另列對照，不得納入總分、排序或入選理由。
...
- 外部共識對照：若使用者要求或資料容易取得，最後才比較券商目標價、分析師評等或媒體共識與本模型差異；此段不得改寫前面的估值、評分或結論。
